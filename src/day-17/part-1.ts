...
const vexist = ({ x, y, z }: Coord) => xexist(x) && xexist(y) && xexist(z);
...
class Space {
  ...
  constructor(source: string) {
    ...
    for (let y = 0; y < layer0.length; y++) {
      for (let x = 0; x < layer0[0].length; x++) {
        this.vmapSet({ x, y, z: 0 }, layer0[y][x]);
      }
    }
    this.printLayer(0);
  }
  ...
  set({ x, y, z }: Coord, value: Cube) {
    const prev = this.map[z][y][x];
  ...
  cycle() {
    ...
    for (let z = bounds.z.min - 1; z <= bounds.z.max + 1; z++) {
      ...
      for (let y = bounds.y.min - 1; y <= bounds.y.max + 1; y++) {
        for (let x = bounds.x.min - 1; x <= bounds.x.max + 1; x++) {
          const c = { x, y, z };
          const prev = this.map[c.z][c.y][c.x];

          const arounds = this.around(c);
          let on = 0;
          for (const a of arounds) {
            if (this.map[a.z][a.y][a.x] === Cube.On) on++;
          }
